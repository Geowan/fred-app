<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
    <div>
        ...
        >
            <div v-if="show_form">
                <checklist-items-form @closeForm="getData();show_form=false;selected = {}" :category="category" :selected="selected"></checklist-items-form>
            </div>
        </v-dialog>
        <v-toolbar dense flat dark color="primary">
            <v-toolbar-title class="subtitle-1 font-weight-bold" v-if="$vuetify.breakpoint.mdAndUp">Checklist items</v-toolbar-title>
            <v-spacer v-if="$vuetify.breakpoint.mdAndUp"></v-spacer>
            <v-btn  @click="selected ={}; show_form = true" class="text-none ml-2" small outlined>Add checklist</v-btn>
        </v-toolbar>
        <v-data-table
                :headers="fields"
                :items="items"
        ...
        >
            <template slot="item" slot-scope="props">
                <tr @click="$emit('onSelected',props.item)" style="cursor:pointer">
                    <td class="caption">
                        {{props.item.no + 1}}
                    </td>
...
<script>
     import ChecklistItemsForm from './ChecklistItemsForm.vue'
    ...
    export default {
        ...
        methods: {
            ...
            getData() {
                ...
                this.$axios.get('/api/administrative/checklist-items?page=' +(this.pagination.page - 1)
            ...
            confirmDelete(){
                ...
                this.$snotify.clear();
                this.$snotify.confirm('Are you sure you want to delete '+ this.selected.name+' checklist', 'Confirmation', {
                    closeOnClick: false,
                    pauseOnHover: true,
                    buttons: [
            ...
            attemptDelete(){
                ...
                this.$snotify.async('Please wait ....', 'Deleting ', () => new Promise((resolve, reject) => {
                    this.$axios.delete("/api/administrative/checklist-items?id="+this.selected.id).then((res) => {
                        resolve({
                            title: 'Success !!!',
